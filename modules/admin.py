...
def mostrar_modal_qr(empleado_datos):
    # Determinar prefijo del código MIN-CARD para asignar colores corporativos suaves
    min_card = str(empleado_datos.get('MIN-CARD', '')).upper().strip()
    
    # Definimos las variables con los nombres EXACTOS (bg, txt, brd)
    if min_card.startswith("AM"):
        bg, txt, brd = "#fef08a", "#1c1917", "#ca8a04"
    elif min_card.startswith("A"):
        bg, txt, brd = "#dbeafe", "#1e3a8a", "#2563eb"
    elif min_card.startswith("V"):
        bg, txt, brd = "#d1fae5", "#064e3b", "#10b981"
    elif min_card.startswith("G"):
        bg, txt, brd = "#f1f5f9", "#334155", "#64748b"
    elif min_card.startswith("M"):
    ...
    datos_para_qr = (
        ...
        f"{linea_restriccion}"
        f"Equipo: {empleado_datos.get('Equipo', 'N/A')}\n"
        f"Tareas: {tareas}"
    )
...
"""
    ...
    st.download_button(
        label="💾 Imprimir / PDF", 
        data=html_impresion, 
        file_name=f"MIN_{empleado_datos.get('MIN-CARD', 'CARD')}.html", 
        mime="text/html", 
...
def render_admin():
    ...
    if "editing_area_id" not in st.session_state:
        st.session_state.editing_area_id = None
    if "editing_turno_id" not in st.session_state:
        st.session_state.editing_turno_id = None
    if "editing_maq_id" not in st.session_state:
        st.session_state.editing_maq_id = None
    if "editing_cargo_id" not in st.session_state:
    ...
    with tab_are:
        ...
        with col_der:
            if st.session_state.editing_area_id is not None:
                st.markdown("##### ✏️ Modificar Área Seleccionada")
                area_id_edit = st.session_state.editing_area_id
                df_area_data = ejecutar_query("SELECT * FROM areas WHERE id = ?", (area_id_edit,), retornar_datos=True)
                ...
                if df_area_data is not None and not df_area_data.empty:
                    ...
                    with st.form("f_edit_area_dinamico"):
                        ...
                        new_rest = st.checkbox("¿Es restringida?", value=bool(datos_area['es_restringida']))
                        
                        col_b1, col_b2 = st.columns(2)
                        with col_b1:
                            if st.form_submit_button("💾 Guardar Cambios"):
                                ejecutar_query("UPDATE areas SET nombre=?, es_restringida=? WHERE id=?", (new_nom.strip(), 1 if new_rest else 0, area_id_edit))
                                st.session_state.editing_area_id = None
        ...
        if df_view is not None and not df_view.empty:
            ...
            grid_head[1].markdown("**Área / Zona**")
            grid_head[2].markdown("**Restringida**")
            ...
            for _, row in df_view.iterrows():
                ...
                grid_row[2].write("🔴 SÍ" if row['es_restringida'] == 1 else "🟢 NO")
    ...
    with tab_tur:
        ...
        with col_der:
            if st.session_state.editing_turno_id is not None:
                st.markdown("##### ✏️ Modificar Turno Seleccionado")
                turno_id_edit = st.session_state.editing_turno_id
                df_turno_data = ejecutar_query("SELECT * FROM turnos WHERE id = ?", (turno_id_edit,), retornar_datos=True)
                ...
                if df_turno_data is not None and not df_turno_data.empty:
                    ...
                    with st.form("f_edit_turno_dinamico"):
                        ...
                        new_ini = st.text_input("Hora Inicio", value=datos_turno['hora_inicio'])
                        new_fin = st.text_input("Hora Fin", value=datos_turno['hora_fin'])
                        
                        col_b1, col_b2 = st.columns(2)
                        with col_b1:
                            if st.form_submit_button("💾 Guardar Cambios"):
                                ejecutar_query("UPDATE turnos SET nombre=?, hora_inicio=?, hora_fin=? WHERE id=?", (new_nom_t.strip(), new_ini, new_fin, turno_id_edit))
                                st.session_state.editing_turno_id = None
                                st.success("Cambios aplicados correctamente.")
                                st.rerun()
                        with col_b2:
        ...
        if df_view is not None and not df_view.empty:
            grid_head = st.columns([1, 3, 2, 2, 2])
            grid_head[0].markdown("**ID**")
            grid_head[1].markdown("**Turno**")
            grid_head[2].markdown("**Horario**")
            grid_head[3].markdown("**Estado**")
            grid_head[4].markdown("**Acción**")
            st.markdown("<hr style='margin: 5px 0px 15px 0px; border-color: gray;'>", unsafe_allow_html=True)
            for _, row in df_view.iterrows():
    ...
    with tab_maq:
        ...
        if df_view is not None and not df_view.empty:
            grid_head = st.columns([1, 3, 2, 2, 2])
            grid_head[0].markdown("**ID**")
            grid_head[1].markdown("**Modelo Equipo**")
            grid_head[2].markdown("**Código Control**")
            grid_head[3].markdown("**Estado**")
            grid_head[4].markdown("**Acción**")
            st.markdown("<hr style='margin: 5px 0px 15px 0px; border-color: gray;'>", unsafe_allow_html=True)
            for _, row in df_view.iterrows():
        ...
        if (areas_comunes is None or areas_comunes.empty) or (turnos is None or turnos.empty) or (maquinas is None or maquinas.empty):
            ...
            if df_lista is not None and not df_lista.empty:
                ...
                for _, row in df_lista.iterrows():
                    ...
                    if row['Activo'] == 1:
                        if col_btn_del.button("🗑️", key=f"del_{row['MIN-CARD']}", help="Inactivar empleado"):
                        ...
                        if col_btn_del.button("🔄", key=f"react_{row['MIN-CARD']}", help="Reactivar empleado"):
                            ejecutar_query("UPDATE empleados SET activo = 1 WHERE uid_tarjeta = ?", (row['MIN-CARD'],))
                            st.toast(f"🔄 Empleado reactivado: {row['Nombre']}")
                            st.rerun()
            else:
                st.info("No hay trabajadores registrados en el sistema.")